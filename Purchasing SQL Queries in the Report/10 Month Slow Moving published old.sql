...
-------Date of Change in the query: 05/4/2023 (as mm/dd/yyyy)

-------Reason for Development: To show 10 month slow inventory items
...
LEFT JOIN (
    P21.dbo.p21_view_invoice_line AS il
    LEFT JOIN P21.dbo.p21_view_invoice_hdr AS ih
    ON il.invoice_no = ih.invoice_no
) ON il.inv_mast_uid = pl.inv_mast_uid
    AND ih.sales_location_id = ph.location_id
    AND ih.date_created between @StartDate and @EndDate
...
AND inl.product_group_id IN ('CIN','ELE','PTP','PVF','HRR','PLB')
...
,network_sales.sales_within_network
,network_sales.qty_sold_within_network
,pl.received_date----------- changed on 4th May 2023 as the filter was wrongly put before and items missing
