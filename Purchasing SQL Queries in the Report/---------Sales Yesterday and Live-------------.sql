...
-------Reason for Development: To develop a metrics of inventory Sales Yesterday and Live inventory report
...
WHERE
 (shrv.invoice_date) >= CAST( GETDATE() AS Date) 

 or CAST(shrv.invoice_date as DATE) = DATEADD(DAY, CASE DATENAME(WEEKDAY, GETDATE())       
                   WHEN 'Sunday' THEN -2 
                   WHEN 'Monday' THEN -3   
