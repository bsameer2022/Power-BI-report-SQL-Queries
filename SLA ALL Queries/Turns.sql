...
(((p21_view_inventory_value_report.qty_on_hand-p21_view_inventory_value_report.special_layer_qty)*p21_view_inventory_value_report.cost)+p21_view_inventory_value_report.special_layer_value) as Value_on_Hand,
--CASE WHEN (((p21_view_inventory_value_report.qty_on_hand-p21_view_inventory_value_report.special_layer_qty)*p21_view_inventory_value_report.cost)+p21_view_inventory_value_report.special_layer_value)  = 0 THEN NULL
--ELSE COALESCE(SUM(shrv.sales_price),0)/((((p21_view_inventory_value_report.qty_on_hand-p21_view_inventory_value_report.special_layer_qty)*p21_view_inventory_value_report.cost)+p21_view_inventory_value_report.special_layer_value) ) END AS Inv_Turns,
...
p21_view_inv_loc.primary_supplier_id
...
LEFT JOIN p21_sales_history_report_view shrv ON  shrv.inv_mast_uid = p21_view_inv_mast.inv_mast_uid AND shrv.branch_id = p21_view_inv_loc.location_id  
LEFT JOIN p21_view_inventory_value_report on p21_view_inventory_value_report.location_id = p21_view_inv_loc.location_id and p21_view_inventory_value_report.inv_mast_uid = p21_view_inv_loc.inv_mast_uid
...
--and p21_view_inv_mast.item_id = '160816'
GROUP BY
...
p21_view_inv_loc.stockable,
p21_view_inventory_value_report.qty_on_hand,
p21_view_inventory_value_report.special_layer_qty,
p21_view_inventory_value_report.cost,
p21_view_inventory_value_report.special_layer_value,
shrv.sales_price
